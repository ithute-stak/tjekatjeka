...
type Dashboard = {
  period: string; sales: number; expenses: number; purchases: number; fuel: number; maintenance: number; operating_spend: number; cash_margin_proxy: number; production_units: number; active_aluminium_jobs: number; active_vehicles: number; low_stock: LowStock[]; finished_stock: FinishedStock[]; branches: Branch[];
};
...
export default async function DashboardPage() {
  const [dashboard, management] = await Promise.all([apiGet<Dashboard>("dashboard", empty), apiGet<ManagementSummary>("management-summary", emptyManagement)]);
  const connected = Boolean(dashboard.period);
  return (
    <Shell>
      <section className="hero">
        <div><span className="eyebrow">Executive overview</span><h1>See what the business is costing — and producing.</h1><p>One live view across Brick Works, Aluminium & Glass, finance and the company fleet. Every operational transaction stays attached to the division that caused it.</p></div>
      ...
      <section className="section-grid">
        <div className="card panel">
          ...
          {dashboard.branches.length ? <div className="branch-grid">{dashboard.branches.map((branch) => (
            <div className="branch-card" key={branch.id}><div className="branch-top"><div><span className="branch-code">{branch.code}</span><h3>{branch.name}</h3></div><span className="eyebrow">{branch.division}</span></div><div className="branch-values"><span>Sales<strong>{money(branch.sales)}</strong></span><span>Expenses<strong>{money(branch.expenses)}</strong></span><span>Purchases<strong>{money(branch.purchases)}</strong></span></div></div>
