...
type Dashboard = {
  ...
  fuel: number;
  maintenance: number;
  operating_spend: number;
  cash_margin_proxy: number;
  production_units: number;
  active_aluminium_jobs: number;
...
export default async function DashboardPage() {
  const dashboard = await apiGet<Dashboard>("dashboard", empty);
    ...
    <Shell>
      <section className="hero">
        <div>
          ...
          <h1>See what the business is costing — and producing.</h1>
          <p>One live view across Brick Works, Aluminium & Glass, finance and the company fleet. Every operational transaction stays attached to the division that caused it.</p>
        </div>
      ...
      <section className="section-grid">
        <div className="card panel">
          <div className="panel-head"><h2>Division performance</h2><span>Current month</span></div>
          {dashboard.branches.length ? <div className="branch-grid">{dashboard.branches.map((branch) => (
            <div className="branch-card" key={branch.id}>
              <div className="branch-top"><div><span className="branch-code">{branch.code}</span><h3>{branch.name}</h3></div><span className="eyebrow">{branch.division}</span></div>
              <div className="branch-values"><span>Sales<strong>{money(branch.sales)}</strong></span><span>Expenses<strong>{money(branch.expenses)}</strong></span><span>Purchases<strong>{money(branch.purchases)}</strong></span></div>
            </div>
          ))}</div> : <div className="empty-state"><strong>No branch transactions yet</strong>Seeded Brick Works and Aluminium branches will appear here once the backend is running.</div>}
        </div>

        <div className="card panel">
          <div className="panel-head"><h2>Attention required</h2><span>Low raw-material stock</span></div>
          {dashboard.low_stock.length ? <div className="stock-list">{dashboard.low_stock.slice(0, 7).map((item) => <div className="stock-row" key={item.id}><strong>{item.name}</strong><span>{number(item.quantity_on_hand)} {item.unit}</span></div>)}</div> : <div className="empty-state"><strong>No low-stock warnings</strong>Materials at or below reorder level will be highlighted here.</div>}
        </div>
      </section>

      <section className="kpi-grid" style={{ marginTop: 18 }}>
        <div className="card kpi"><div className="kpi-head"><span>Aluminium jobs</span><span className="kpi-icon">A</span></div><strong>{number(dashboard.active_aluminium_jobs, 0)}</strong><small>Open fabrication / installation jobs</small></div>
        <div className="card kpi"><div className="kpi-head"><span>Active vehicles</span><span className="kpi-icon">V</span></div><strong>{number(dashboard.active_vehicles, 0)}</strong><small>Trucks and small vehicles in operation</small></div>
      ...
      <section className="module-grid">
        {modules.map(([href, icon, title, copy]) => <Link className="card module-card" href={href} key={href}><span className="mini-icon">{icon}</span><h3>{title}</h3><p>{copy}</p></Link>)}
      </section>
    </Shell>
  );
}
